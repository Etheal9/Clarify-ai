...
export const VerifySection: React.FC<VerifySectionProps> = ({ data, isLoading, onVerify, hasInput }) => {
  if (isLoading) {
    return (
      <div className="p-8 space-y-4 animate-pulse bg-white dark:bg-gray-800 h-full transition-colors">
  ...
  if (!data) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-gray-800 transition-colors">
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 transition-colors">
      <div className="p-6 border-b border-gray-100 dark:border-gray-700 bg-green-50/50 dark:bg-green-900/10">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {data.sources.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-3">Sources</h4>
            <div className="grid gap-2">
              {data.sources.map((source, idx) => (
                <a 
                  ...
                  className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-sm transition-all group"
                >
                  <span className="text-sm text-gray-700 dark:text-gray-200 font-medium truncate flex-1 mr-4">{source.title}</span>
